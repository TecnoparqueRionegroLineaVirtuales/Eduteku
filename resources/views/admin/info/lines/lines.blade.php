            ...
            <div id="main" class="main-content mt-12 md:mt-2 pb-24 md:pb-5">
                <div class="bg-gray-100 pt-3">
                    <div class="rounded-tl-3xl bg-gradient-to-r from-gray-100 to-green-500 p-4 shadow text-2xl text-current">
                        <h1 class="font-bold pl-2">Lineas</h1>
                    </div>
                </div>
                @if (session('success'))
              ...
              @endif
                <div class="pt-10 py-10 px-10">
                    <button id="openModal" class="inline-flex items-center bg-green-500 border-0 py-1 px-3 focus:outline-none hover:bg-green-500 text-white rounded text-base mt-4 md:mt-0">Agregar Linea</button>
                </div>
                                ...
                                <table class="min-w-full text-left text-sm font-light text-surface">
                                  ...
                                  <tbody>
                                    ...
                                    <tr class="border-b border-neutral-200">
                                        <td class="whitespace-nowrap px-6 py-4 font-medium">{{ $multimedias->id }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4">{{ $multimedias->category->name }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4">
                                            <form action="{{ route('multimedia.destroy', $multimedias->id) }}" method="POST">
                                                ...
                                                <a href="{{ route('lines.edit', $multimedias->id) }}" class="bg-gray-400 text-white p-2 rounded"><i class="fa fa-pen"></i></a>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
                                ...
                                <div id="modal-container" class="modal-container inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all mt-12 sm:align-middle sm:max-w-lg w-full">
                                    <div class="modal-wrapper bg-white px-4 pt-5 pb-4 sm:p-6 sm:pd-4">
                                        <div class="modal-wrapper-flex sm:flex sm:item-start">
                                            <div class="modal-icon mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-500 sm:mx-0 sm:h-10 sm:w-10"><i class="fa fa-info fa-2x fa-inverse"></i></div>
                                            <div class="modal-content text-center mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                                <h3 class="text-lg font-medium text-gray-900">Agregar Linea</h3>
                                                <div class="modal-text">
                                                <form action="{{ route('lines.store') }}" method="post" enctype="multipart/form-data">
                                                    @csrf
