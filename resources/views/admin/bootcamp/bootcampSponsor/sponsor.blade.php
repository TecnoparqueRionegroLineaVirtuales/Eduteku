            ...
            <div id="main" class="main-content mt-12 md:mt-2 pb-24 md:pb-5">
                <div class="bg-gray-100 pt-3">
                    <div class="rounded-tl-3xl bg-gradient-to-r from-gray-100 to-green-500 p-4 shadow text-2xl text-current">
                        <h1 class="font-bold pl-2">Instituciones</h1>
                    </div>
                </div>
                @if (session('success'))
              ...
              @endif
                <div class="pt-10 py-10 px-10">
                    <button id="openModal" class="inline-flex items-center bg-green-500 border-0 py-1 px-3 focus:outline-none hover:bg-green-500 text-white rounded text-base mt-4 md:mt-0">Agregar Instituciones</button>
                </div>
                                ...
                                <table class="min-w-full text-left text-sm font-light text-surface">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="border-b border-neutral-200">
                                            <td class="whitespace-nowrap px-6 py-4 font-medium">{{ $sponsors->id }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 w-1/4">{{ $sponsors->name }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 w-1/4">{{ $sponsors->description }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 w-1/4">
                                                @if ($sponsors->img_url)
                                                    <img src="{{ asset('storage/img/'.$sponsors->img_url) }}" alt="{{ $sponsors->name }}" class="w-20 h-20 object-cover">
                                                @else
                                                    No Image
                                                @endif
                                            </td>
                                            <td class="whitespace-nowrap px-6 py-4">
                                                <form action="{{ route('sponsor.destroy', $sponsors->id) }}" method="POST" id="deleteForm_{{ $sponsors->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="bg-red-400 text-white p-2 rounded" onclick="return confirm('¿Estás seguro de que deseas eliminar la Institucion?');"><i class="fa fa-trash"></i></button>
                                                    <a href="{{ route('sponsor.edit', $sponsors->id) }}" class="bg-gray-400 text-white p-2 rounded"><i class="fa fa-pen"></i></a>
                                                </form>
                                            </td>
                                        </tr>
                                        @endforeach
                                    ...
                                    <div class="modal-wrapper bg-white px-4 pt-5 pb-4 sm:p-6 sm:pd-4">
                                        <div class="modal-wrapper-flex sm:flex sm:item-start">
                                            ...
                                            <div class="modal-content text-center mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                                <h3 class="text-lg font-medium text-gray-900">Agregar Instituciones</h3>
                                                    ...
                                                    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
                                                        ...
                                                        <div class="-mx-3 md:flex mb-6">
                                                          <div class="md:w-full px-3">
                                                            ...
                                                            <input class="w-full bg-gray-200 text-black border border-gray-200 rounded py-3 px-4 mb-3" id="application-link" type="text" name="description" placeholder="Descripción">
                                                          </div>
                                                        </div>
                                                        <div class="-mx-3 md:flex mb-6">
                                                          <div class="md:w-full px-3">
