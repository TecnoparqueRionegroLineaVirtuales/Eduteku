                    ...
                    <div class="modal-wrapper bg-white px-4 pt-5 pb-4 sm:p-6 sm:pd-4">
                        <div class="modal-wrapper-flex sm:flex sm:item-start">
                            ...
                            <div class="modal-content text-center mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg font-medium text-gray-900">Boletin</h3>
                                <div class="modal-text">
                                    <form action="{{ route('bulletinAdmin.update', $multimedias->id) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
                                            <div class="-mx-3 md:flex mb-6">
                                                <div class="md:w-full px-3">
                                                    ...
                                                    <input class="w-full bg-gray-200 text-black border border-gray-200 rounded py-3 px-4 mb-3" id="name" type="text" name="name" placeholder="Nombre" value="{{ $multimedias->name }}">
                                                </div>
                                            </div>
                                            <div class="-mx-3 md:flex mb-6">
                                                <div class="md:w-full px-3">
                                                    <label class="uppercase tracking-wide text-black text-xs font-bold mb-2" for="descripcion">Descripción</label>
                                                    <textarea class="w-full bg-gray-200 text-black border border-gray-200 rounded py-3 px-4 mb-3" name="descripcion" id="descripcion" placeholder="Descripción">{{ $multimedias->descripcion }}</textarea>
                                                </div>
                                            </div>
                                            <div class="-mx-3 md:flex mb-6">
                                                <div class="md:w-full px-3">
                                                    <label class="uppercase tracking-wide text-black text-xs font-bold mb-2" for="url">Archivo</label>
                                            ...
                                            <div class="-mx-3 md:flex mb-6">
                                                <div class="md:w-full px-3">
                                                    ...
                                                    <textarea class="w-full bg-gray-200 text-black border border-gray-200 rounded py-3 px-4 mb-3" name="link" id="application-link" placeholder="Link">{{ $multimedias->link }}</textarea>
                                                </div>
                                            </div>
                                            <div class="modal-actions bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                                ...
                                                <a href="{{ route('bulletinAdmin.index') }}" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-md px-4 py-2 bg-red-400 font-medium text-gray-50 hover:bg-gray-700 hover:text-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Cancelar</a>
                                            </div>
                                        </div>
                                    </form>
                                </div>
